...
class Pipeline:
    ...
    def _split_data(self):
        ...
        # Check that input vectors are available before proceeding
        if not self._input_vectors:
            raise ValueError("No input vectors to split. Ensure _preprocess_features has populated input data.")
    ...
    def save(self):
        """
        Saves the pipeline.
        Serializes the pipeline object and dataset to bytes, then stores it in the registry.
        """
        ...
        automl.registry.save_data(pipeline_bytes, type="pipeline", name=pipeline_id)
        automl.registry.save_data(dataset_bytes, type="dataset", name=dataset_id)

        print(f"Pipeline saved successfully with name: {self._dataset['name']}")
